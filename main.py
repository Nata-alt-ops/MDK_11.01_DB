...
def check_table_structure():
    ...
    if conn:
        try:
            ...
            for row in columns:
                print(f"Столбец: {row[0]}, Тип: {row[1]}, Nullable: {row[2]}")
...
# Проверим структуру при запуске
booking_columns = check_table_structure()
print("Доступные столбцы в Bookings:", booking_columns)
...
def register_user():
    ...
    center_window(register_window, 350, 200)  # Используем функцию центрирования

    Label(register_window, text="Новое имя пользователя:").pack(pady=5)
    entry_new_username = Entry(register_window)
    entry_new_username.pack(pady=5)

    Label(register_window, text="Новый пароль:").pack(pady=5)
    entry_new_password = Entry(register_window, show='*')
    entry_new_password.pack(pady=5)

    Button(register_window, text="Зарегистрироваться", command=submit_registration, bg='lightgreen').pack(pady=10)

    label_register_status = Label(register_window, text="")
...
def open_main_window(user_type, username):
    ...
    center_window(main_window, 600, 500)  # Центрируем главное окно
...
def view_all_rooms():
    ...
    center_window(rooms_window, 700, 550)  # Центрируем окно комнат
...
def book_room(room_id, room_number):
    ...
    center_window(booking_window, 300, 250)  # Центрируем окно бронирования
...
def view_my_bookings(username):
    ...
    center_window(bookings_window, 700, 450)  # Центрируем окно бронирований
...
def clear_fields():
    ...
    entry_username.delete(0, END)
    entry_password.delete(0, END)
    label_status.config(text="Поля очищены")

# Основное окно авторизации
root = Tk()
root.title("Авторизация в отеле")
center_window(root, 400, 600)
root.configure(bg='lightblue')


# Заголовок
Label(root, text="Система управления отелем", 
...
Label(input_frame, text="Логин:").grid(row=0, column=0, sticky=W, pady=5)
entry_username = Entry(input_frame, width=20)
entry_username.grid(row=0, column=1, pady=5, padx=5)

Label(input_frame, text="Пароль:").grid(row=1, column=0, sticky=W, pady=5)
entry_password = Entry(input_frame, show='*', width=20)
entry_password.grid(row=1, column=1, pady=5, padx=5)

# Кнопки основного управления
button_frame = Frame(root)
button_frame.pack(pady=15)

Button(button_frame, text="Войти", command=authenticate_user, 
       bg='lightblue', width=12, height=2).grid(row=0, column=0, padx=5)
Button(button_frame, text="Регистрация", command=register_user, 
       bg='lightgreen', width=12, height=2).grid(row=0, column=1, padx=5)

# Кнопка очистки
Button(button_frame, text="Очистить", command=clear_fields, 
       bg='lightyellow', width=12, height=2).grid(row=0, column=2, padx=5)

# Статус
label_status = Label(root, text="Введите данные для входа", fg='blue')
